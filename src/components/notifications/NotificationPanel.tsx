...
import { AcademicNotification, NotificationType } from '../../types';

export const INITIAL_NOTIFICATIONS: AcademicNotification[] = [
  {
    id: 'notif-1',
    title: 'Examen Parcial de Farmacología I',
    description: 'Viernes 08:30 hs en el Aula Magna FCM. Temas: Farmacocinética, Receptores y Sistema Autónomo.',
    type: 'exam',
  ...
  {
    id: 'notif-2',
    title: 'Entrega de Historia Clínica (Semiología)',
    description: 'Recordatorio: Subir el informe del caso clínico al portal de cátedra antes de las 23:59 hs.',
    type: 'assignment',
    timestamp: 'Hace 2 horas',
    date: 'Hoy, 23:59',
    isRead: false,
    priority: 'high',
    tag: 'Semiología Médica',
  },
  {
    id: 'notif-3',
    title: 'Jornada Universitaria de Salud & Vacunación',
    description: 'Convocatoria abierta para estudiantes de 2do y 3er año en el Pabellón Quirúrgico FCM este jueves.',
    type: 'campus',
    timestamp: 'Hace 5 horas',
    date: 'Jueves 10:00',
    isRead: false,
    priority: 'medium',
    tag: 'Aviso Campus FCM',
  },
  {
    id: 'notif-4',
    title: 'Alerta de Asistencia en Fisiopatología',
    description: 'Tu asistencia actual es de 84.3%. Recuerda justificar inasistencia médica del lunes pasado en secretaría.',
    type: 'attendance',
    timestamp: 'Ayer',
    date: 'Ayer, 16:40',
    isRead: true,
  ...
  {
    id: 'notif-5',
    title: 'Laboratorio de Neuroanatomía Práctica',
    description: 'Traer guardapolvo blanco, guantes estériles y kit de disección para la sesión práctica de corte cerebral.',
    type: 'assignment',
...
interface NotificationPanelProps {
  ...
  setNotifications: React.Dispatch<React.SetStateAction<AcademicNotification[]>>;
}
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (panelRef.current && !panelRef.current.contains(e.target as Node)) {
        // Only close if not clicking the bell trigger (which handles toggle)
  ...
  const getNotificationColorStyles = (type: NotificationType, isRead: boolean) => {
    if (isRead) {
      return {
        ...
        cardBg: 'bg-white/50 border-white/60 hover:bg-white/80',
        iconBg: 'bg-white/80',
      };
    }
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute right-3 sm:right-8 top-16 w-[calc(100vw-24px)] sm:w-[420px] max-h-[85vh] flex flex-col rounded-[26px] bg-white/80 backdrop-blur-2xl border border-white/90 shadow-2xl shadow-[#864e5a]/20 z-50 overflow-hidden text-[#1b1c1c]"
        >
          {/* Header */}
          <div className="p-4 sm:p-5 border-b border-black/5 flex items-center justify-between bg-gradient-to-b from-white/90 to-white/60">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#514345]/80 font-medium">
                  Notificaciones y alertas académicas FCM
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-xl bg-white/80 hover:bg-[#cde9ac] text-[#4e6535] text-[11px] font-bold border border-white shadow-2xs transition-all flex items-center gap-1"
                >
          ...
          <div className="px-4 py-2.5 bg-black/[0.02] border-b border-black/5 flex items-center gap-1.5 overflow-x-auto no-scrollbar text-xs">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-xl font-semibold whitespace-nowrap transition-all text-[11px] ${
                activeFilter === 'exam'
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5 max-h-[50vh] min-h-[140px]">
            {filteredNotifications.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center text-[#514345] space-y-2">
                <div className="w-12 h-12 rounded-2xl bg-white/70 border border-white flex items-center justify-center text-[#864e5a] shadow-inner">
                  <BookmarkCheck className="w-6 h-6 text-[#6ca561]" />
                </div>
                ...
                <p className="text-xs text-[#514345]/70 max-w-[240px]">
                  {notifications.length === 0
                    ? 'Has descartado todas las notificaciones.'
                    : 'Puedes ver todas las notificaciones cambiando el filtro superior.'}
                </p>
                {notifications.length === 0 && (
                  <button
                    onClick={handleRestoreDefaults}
                    className="mt-2 px-3 py-1.5 rounded-xl bg-[#cde9ac] hover:bg-[#b8da93] text-[#374d20] text-xs font-bold transition-colors flex items-center gap-1.5 shadow-2xs"
                  >
                    <RotateCcw className="w-3.5 h-3.5" /> Restaurar avisos de muestra
                  </button>
                )}
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-[#514345] hover:text-[#ba1a1a] hover:bg-black/5 transition-all"
          ...
          {notifications.length > 0 && (
            <div className="p-3 px-4 border-t border-black/5 bg-white/70 flex items-center justify-between text-xs text-[#514345]">
              <span className="text-[11px] font-medium">
                {unreadCount > 0 ? (
                  ...
                  <span className="text-[#374d20] font-bold flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#4e6535]" /> Al día
                  </span>
                )}
              </span>
              ...
              <button
                ...
                className="text-[11px] font-bold text-[#ba1a1a] hover:underline flex items-center gap-1 transition-opacity"
              >
