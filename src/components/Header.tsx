...
import { Sparkles, HeartPulse, Clock, Menu, Bell, Headphones, LogOut, User } from 'lucide-react';
import { NotificationPanel, INITIAL_NOTIFICATIONS } from './notifications/NotificationPanel';
import { SpotifyPlayerModal, PRESET_PLAYLISTS, SpotifyPlaylistPreset } from './spotify/SpotifyPlayerModal';
import { AcademicNotification, AuthUser } from '../types';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [activeSpotifyPlaylist, setActiveSpotifyPlaylist] = useState<SpotifyPlaylistPreset>(() => {
    try {
      const saved = localStorage.getItem('campusbloom_active_spotify_playlist');
      return saved ? JSON.parse(saved) : PRESET_PLAYLISTS[0];
    } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('campusbloom_active_spotify_playlist', JSON.stringify(activeSpotifyPlaylist));
    } catch {
      // ignore
    }
  ...
  const [notifications, setNotifications] = useState<AcademicNotification[]>(() => {
    try {
      const saved = localStorage.getItem('campusbloom_notifications');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('campusbloom_notifications', JSON.stringify(notifications));
    } catch {
      // ignore
    }
  }, [notifications]);
  ...
  return (
    <header id="campusbloom-header" className="w-full flex items-center justify-between px-4 sm:px-8 py-3.5 z-30 relative">
        ...
        <div className="flex items-center gap-3.5 group cursor-pointer" onClick={onOpenBackgroundModal}>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#ffb7c5]/40 text-[#7b4551] border border-[#ffb7c5]/60">
                FCM • Medicina
              </span>
            </div>
          ...
          >
            ...
            <svg
              className={`w-4 h-4 transition-colors ${
                isSpotifyOpen ? 'text-[#1DB954]' : 'text-[#1DB954]'
              } fill-current`}
              viewBox="0 0 24 24"
            >
        ...
        {currentUser && (
          <div className="flex items-center gap-1.5 pl-1">
            <button
              ...
              className="flex items-center gap-2 px-2.5 py-1.5 rounded-2xl bg-white/80 hover:bg-white text-[#1b1c1c] border border-white/90 shadow-2xs transition-all hover:scale-105"
            >
            ...
            {onLogout && (
              <button
                ...
                onClick={onLogout}
                title="Cerrar Sesión"
                className="p-2 rounded-2xl bg-white/70 hover:bg-[#ffdad6] text-[#837375] hover:text-[#ba1a1a] border border-white/80 transition-all"
              >
      ...
      <SpotifyPlayerModal
        ...
        activePlaylist={activeSpotifyPlaylist}
        onSelectPlaylist={(p) => setActiveSpotifyPlaylist(p)}
      />
